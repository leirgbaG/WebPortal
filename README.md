# Web Portal

Dynamic Web Portal for the SACN (Spanish acronym for Alimentary Security and Nutritional Culture) Department on [Territorial Polytechnic University of Paria "Luis Mariano Rivera"](https://uptparia.edu.ve/).

This allows the Department upload News and Importants Documents for the SACN students.

![Badge Terminado](https://img.shields.io/badge/Status-FINISHED-blue)

## 📲 Installation

Use a PHP interpreter like [XAMPP](https://www.apachefriends.org/):

1. Open XAMPP.
2. Start Apache.
3. Start MySQL.
4. Open [PHPMyAdmin](localhost/phpmyadmin/).
5. Create a new user: `user: root`; `password: starlight.21`.
6. Import the `portalweb_sacn (1).sql` file from `db` directory on PHPMyAdmin.
7. Move the repository to the `htdocs` directory created on the XAMPP directory (Common path: C:/xampp/htdocs/)
8. Open [localhost/PortalWeb/](localhost/PortalWeb) on web.

High level user: `user: gabigabs123`; `password: gabPowered`.

## 💻 Technologies

- ![JavaScript](https://img.shields.io/badge/-JavaScript-333333?logo=javascript)
- ![HTML5](https://img.shields.io/badge/-HTML5-333333?logo=HTML5)
- ![CSS3](https://img.shields.io/badge/-CSS3-333333?logo=CSS&logoColor=663399)
- ![PHP8](https://img.shields.io/badge/-PHP8-333333?logo=php&logoColor=777bb4)
- ![MySQL](https://img.shields.io/badge/-MySQL-333333?logo=mysql&logoColor=#4479A1)
- ![XAMPP](https://img.shields.io/badge/-XAMPP-333333?logo=xampp&logoColor=fb7a24)

## 🪪 License

[Mozilla Public License 2.0](https://choosealicense.com/licenses/mpl-2.0/)

![Static Badge](https://img.shields.io/badge/License-Mozilla_Public_License_2.0-orange)

## 📩 Contact Me

![X (formerly Twitter) Follow](https://img.shields.io/twitter/follow/Gab__To)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Eduardo_González-0a66c2?logoColor=ffffff&logo=data:image/svg+xml;base64,)](https://www.linkedin.com/in/eduardo-gonz%C3%A1lez-b4a6b5270/)

[![Gmail](https://img.shields.io/badge/Gmail-odraudegonzalezm@gmail.com-EA4335?logo=gmail)](mailto:odraudegonzalezm@gmail.com?subject=Saludos%20Gabriel)
